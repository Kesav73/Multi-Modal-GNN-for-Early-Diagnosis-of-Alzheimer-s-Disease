...
PHENOTYPIC_CSV = "./data/phenotypic/phenotypic_with_mmse_apoe4.csv"
...
print(f"Phase2 subjects : {len(subj_ids)}")
...
apoe4s = pheno["APOE4"].values.astype(str)
...
# ─────────────────────────────────────────────────────────────────
#  STEP 3: Adjacency Helper Functions
#  (Adjacency is recomputed inside each task loop)
# ─────────────────────────────────────────────────────────────────
print("\n" + "=" * 65)
print("STEP 3: Preparing Adjacency Helper Functions")
print("=" * 65)
...
# ─────────────────────────────────────────────────────────────────
print("\n" + "=" * 65)
print("STEP 8: Per-task Adjacency + Train / Val / Test Splits  (70/15/15, stratified)")
print("=" * 65)
...
for task_name, cfg in TASK_CONFIGS.items():
    ...
    # Restrict graph construction to this task's subjects only.
    X_s = X_smri[task_mask]
    X_p = X_pet[task_mask]
    g_t = genders[task_mask]
    m_t = mmscores[task_mask]
    a_t = apoe4s[task_mask]

    # Eq.6: similarity matrices on task subjects only.
    A_s, sig_s = similarity_adjacency(X_s, sigma=None, name=f"sMRI-{task_name}")
    A_p, sig_p = similarity_adjacency(X_p, sigma=None, name=f"PET-{task_name}")

    # Eq.8/Eq.9/Eq.10: task-specific phenotypic indicator matrices.
    r_G_t = (g_t[:, None] == g_t[None, :]).astype(np.float32)
    r_P_t = (a_t[:, None] == a_t[None, :]).astype(np.float32)
    r_M_t = (np.abs(m_t[:, None] - m_t[None, :]) <= 1).astype(np.float32)
    pw = r_G_t + r_P_t + r_M_t

    A_s_ph = A_s * pw
    A_p_ph = A_p * pw
    np.fill_diagonal(A_s_ph, 1.0)
    np.fill_diagonal(A_p_ph, 1.0)

    A_im = A_s_ph * A_p_ph
    np.fill_diagonal(A_im, 1.0)

    X_cat = np.concatenate([X_s, X_p], axis=1)
    A_fm, _ = similarity_adjacency(X_cat, sigma=(sig_s + sig_p) / 2.0, name=f"Fuse-{task_name}")
    A_fm = A_fm * pw
    ...
    print(f"  A_if density : {(A_if[~np.eye(len(task_idx), dtype=bool)]>0).mean():.4f}")
    print(f"  A_if_norm symmetric: {np.allclose(A_if_norm, A_if_norm.T, atol=1e-5)}")
    ...
    np.save(os.path.join(task_dir, "A_if_norm.npy"), A_if_norm.astype(np.float32))
    np.save(os.path.join(task_dir, "X_smri.npy"), X_s.astype(np.float32))
    np.save(os.path.join(task_dir, "X_pet.npy"), X_p.astype(np.float32))
    ...
    print(f"  Saved to {task_dir}/ (A_if_norm, X_smri, X_pet, masks, labels)")
...
for task_name, g in task_graphs.items():
    print(f"\n  ── Task graph stats: {task_name} ──")
    graph_stats(g["A_s"], "A_s         (task sMRI similarity)")
    graph_stats(g["A_p"], "A_p         (task PET similarity)")
    graph_stats(g["A_im"], "A_im        (Eq.11  sMRI⊙PET)")
    graph_stats(g["A_fm"], "A_fm        (Eq.12  fusion)")
    graph_stats(g["A_if"], "A_if        (Eq.13  final)")
    graph_stats(g["A_if_norm"], "A_if_norm   (normalized  ← GNN)")

    print("\n  Mean edge weight between same-class vs different-class pairs (A_if):")
    labels_task = g["labels_s"]
    A_if_task = g["A_if"]
    for lbl in sorted(set(labels_task.tolist())):
        m = labels_task == lbl
        if not m.any():
            continue
        same_class = A_if_task[np.ix_(m, m)].copy()
        other_class = A_if_task[np.ix_(m, ~m)]
...
files = [
    ...
    ("AD_vs_CN/A_if_norm.npy",           "Task-specific normalized adjacency"),
    ("AD_vs_CN/X_smri.npy",              "Task-specific sMRI features"),
    ("AD_vs_CN/X_pet.npy",               "Task-specific PET features"),
    ...
    ("SMCI_vs_PMCI/A_if_norm.npy",       "Task-specific normalized adjacency"),
    ("SMCI_vs_PMCI/X_smri.npy",          "Task-specific sMRI features"),
    ("SMCI_vs_PMCI/X_pet.npy",           "Task-specific PET features"),
...
print("  GNN inputs:")
print("  • AD_vs_CN/X_pet.npy + AD_vs_CN/X_smri.npy")
print("  • AD_vs_CN/A_if_norm.npy    (task-specific normalized adjacency)")
print("  • AD_vs_CN/                 (labels + masks for task 1)")
print("  • SMCI_vs_PMCI/X_pet.npy + SMCI_vs_PMCI/X_smri.npy")
print("  • SMCI_vs_PMCI/A_if_norm.npy (task-specific normalized adjacency)")
